...
from tapas.models import tf_utils
...
class BertModel(tf.keras.layers.Layer):
    ...
    def build(self, unused_input_shapes):
        ...
        self.embedding_postprocessor = EmbeddingPostprocessor(
            ...
            reset_position_index_per_cell=self.config.reset_position_index_per_cell,
            initializer_range=self.config.initializer_range,
            dtype=tf.float32,
    ...
    def __call__(self, input_word_ids, input_mask=None, segment_ids=None,
                 column_ids=None, row_ids=None, prev_label_ids=None, column_ranks=None,
                 inv_column_ranks=None, numeric_relations=None, ** kwargs):
        inputs = tf_utils.pack_inputs(
            [input_word_ids, input_mask, segment_ids, column_ids,
             row_ids, prev_label_ids, column_ranks,
...
class EmbeddingPostprocessor(tf.keras.layers.Layer):
    ...
    def build(self, input_shapes):
        """Implements build() for the layer."""
        (word_embeddings_shape, _,_,_,_,_,_,_) = input_shapes
        width = word_embeddings_shape.as_list()[-1]
        self.type_embeddings = None
        if self.use_type_embeddings:
            self.segment_embeddings = self.add_weight(
                "segment_embeddings",
                shape=[self.token_type_vocab_size[0], width],
                initializer=get_initializer(self.initializer_range),
                dtype=self.dtype)
            self.column_embeddings = self.add_weight(
                "column_embeddings",
                shape=[self.token_type_vocab_size[1], width],
                initializer=get_initializer(self.initializer_range),
                dtype=self.dtype)
            self.row_embeddings = self.add_weight(
                "row_embeddings",
                shape=[self.token_type_vocab_size[2], width],
                initializer=get_initializer(self.initializer_range),
                dtype=self.dtype)
            self.prev_label_embeddings = self.add_weight(
                "prev_label_embeddings",
                shape=[self.token_type_vocab_size[3], width],
                initializer=get_initializer(self.initializer_range),
                dtype=self.dtype)
            self.column_ranks_embeddings = self.add_weight(
                "column_ranks_embeddings",
                shape=[self.token_type_vocab_size[4], width],
                initializer=get_initializer(self.initializer_range),
                dtype=self.dtype)
            self.inv_column_ranks_embeddings = self.add_weight(
                "inv_column_ranks_embeddings",
                shape=[self.token_type_vocab_size[5], width],
                initializer=get_initializer(self.initializer_range),
                dtype=self.dtype)
            self.numeric_relations_embeddings = self.add_weight(
                "numeric_relations_embeddings",
                shape=[self.token_type_vocab_size[6], width],
                initializer=get_initializer(self.initializer_range),
                dtype=self.dtype)
    ...
    def __call__(self, word_embeddings, segment_ids=None, column_ids=None, row_ids=None,
                 prev_label_ids=None, column_ranks=None, inv_column_ranks=None,
                 numeric_relations=None, ** kwargs):
        inputs = tf_utils.pack_inputs([word_embeddings, segment_ids, column_ids,
                                       row_ids, prev_label_ids, column_ranks,
                                       inv_column_ranks, numeric_relations])
